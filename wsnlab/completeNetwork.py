...
class SensorNode(wsn.Node):
    ...
    def repair_find_another_parent(self):
        """If it has potential parent in its table, tries to connect any of them. Otherwise, becomes unregistered.
           The algorithm resembles select_and_join function in its attempt to reconnect failed nodes.
        ...
        if len(self.candidate_parents_table) != 0:
            for gui in self.candidate_parents_table:
                if self.neighbors_table[gui]['hop_count'] < min_hop or (self.neighbors_table[gui]['hop_count'] == min_hop and gui < min_hop_gui):
                    if self.neighbors_table[gui]['role'] == Roles.CLUSTER_HEAD and self.role == Roles.REGISTERED:
                        min_hop = self.neighbors_table[gui]['hop_count']
                        min_hop_gui = gui
                    elif self.ch_addr is not None and self.neighbors_table[gui]['role'] == Roles.REGISTERED:
                        min_hop = self.neighbors_table[gui]['hop_count']
                        min_hop_gui = gui
            if min_hop_gui != 9999 and min_hop < self.hop_count:
                self.send_join_request(self.neighbors_table[min_hop_gui]['source'])
            else:
                self.send_i_am_orphan()
                self.become_unregistered()
        else:
